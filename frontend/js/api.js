...
// ----------------------------- ↓ EVENT LISTENERS (TRIGGERS) FOR TASKS ↓ ----------------------------------
    ...
    toDoList.addEventListener("click", (event) =>{
        if (event.target.classList.contains("edit")){
            ...
            //This is to format the due date
...
// --------------------------------------------- ↓ TASK FUNCTIONS ↓ ---------------------------------------------
        ...
        async function displayTasks() {
            try {
                ...
                const response = await fetch(`${URL}/tasks${query}`);  // this is related to a get request. doesn't need further information/settings.
                ...
                function formatTask(task) {
                    ...
                    li.innerHTML = task.completed ? //will result in a true answer or false answer for an output below
                    `
                ...
                resetForm(); //resets the new task form
        ...
        async function createNewTask() {
            // const taskDetails = {
            //     id: 5,
            //     title: document.getElementById("taskName").value.trim(),      //trim takes care of any spaces before or after the entry
            ...
            try {
                ...
                const response = await fetch(`${URL}/tasks/todo`, {     //post requests (and all other requests) need further settings to be defined (unlike get requests)
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"     // this is specifying the data type 
                    },
                    body: JSON.stringify(taskDetails)   // this is a request body seen as req.body on index.js. during transit, translate the json data to a string
                });

                if(!response.ok) {
                    throw new Error(`Failed to create a new task: ${response.status}`)     //this is called error logging
                } 
       ...
       async function editTask(id) {
        try {
            const updatedDetails = {
                ...
                description: document.getElementById("editTaskName").value.trim(),
    ...
    //    async function getExample() {     // async....await   multiple waiters serving coffee
    //     const reponse = await fetch("http://localhost:3000/get/example");
